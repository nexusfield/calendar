...
def compose_scripture_and_devotional() -> tuple:
    ...
    message = client.messages.create(
        ...
        messages=[{"role": "user", "content": """Pick any passage from the Bible fitting for a man starting his day. Write the most striking verse or two in modern plain English. Then write a 3-4 sentence men's devotional on it — the kind that puts steel in a man's spine before he walks out the door. Grounded in the character of God and the person of Christ. Direct, honest, no filler.
...
{"scripture_ref": "...", "scripture": "...", "devotional": "..."}"""}],
    ...
    return result.get("scripture_ref", ""), result.get("scripture", ""), result.get("devotional", "").replace("\u2014", "-")
